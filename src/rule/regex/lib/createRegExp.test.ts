...
describe(createRegExp.name, () => {
  ...
  test("ignore duplicate flags", () => {
    expect(
      createRegExp("\\d", [RegexFlags.multiLine, RegexFlags.multiLine]),
    ).toStrictEqual(RegExp("\\d", "m"));
  });
  test("setGlobal", () => {
    expect(createRegExp("\\d", [RegexFlags.multiLine], true)).toStrictEqual(
      RegExp("\\d", "gm"),
    );
  });
  test("setGlobal + global flag", () => {
    expect(createRegExp("\\d", [RegexFlags.global], true)).toStrictEqual(
      RegExp("\\d", "g"),
    );
  });
});
