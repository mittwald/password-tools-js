...
describe(`${SequenceRule.name}.validatePassword()`, () => {
  describe("default maxLength = 3", () => {
    testBySequenceTypes(SequenceType.repeat, [
      { validSequences: ["333"], invalidSequences: ["4444"] },
    ]);
    testBySequenceTypes(SequenceType.keyboard, [
      { validSequences: ["asd"], invalidSequences: ["asdf"] },
    ]);
    testBySequenceTypes(SequenceType.number, [
      { validSequences: ["123"], invalidSequences: ["1234"] },
    ]);
    testBySequenceTypes(SequenceType.alphabet, [
      { validSequences: ["abc"], invalidSequences: ["abcd"] },
    ]);
  });
  describe("maxLength", () => {
    testBySequenceTypes(SequenceType.repeat, [
      { validSequences: ["22"], invalidSequences: ["333"], maxLength: 2 },
    ]);
  });
  describe("multiple sequenceTypes", () => {
    testRule(
      new SequenceRule({
        sequences: [SequenceType.alphabet, SequenceType.keyboard],
      }),
      [
        {
          pw: "_asd_abc_",
