...
interface TestFunctionsThatFindSequencesProps {
  theSequenceFindingFunctionWeWantToTest: (
    pw: string,
    maxLength: number,
  ) => string[];
  maxLength?: number;
...
const testFunctionsThatFindSequences = (
  props: TestFunctionsThatFindSequencesProps,
): void => {
  ...
  describe(theSequenceFindingFunctionWeWantToTest.name, () => {
    ...
    test(`return [], because no sequence > maxLength of ${maxLength} in pw: '${validPw}'`, () => {
      expect(
        theSequenceFindingFunctionWeWantToTest(validPw, maxLength),
      ).toStrictEqual([]);
    });
    test(`return ${invalidSequences}, because they exceed the maxLength of ${maxLength} in pw: '${invalidPw}'`, () => {
      expect(
        theSequenceFindingFunctionWeWantToTest(invalidPw, maxLength),
      ).toStrictEqual(invalidSequences);
    });
  });
};
